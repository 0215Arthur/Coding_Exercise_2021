...
- 根据以上定义可知： 二叉树中最小值为root节点，通过深度搜索，每次找到比root节点的值即可
  - 当搜索到比root节点大的值即刻返回
...
public:
    ...
    void search(TreeNode* root,  TreeNode* last, int k, int depth) {
        // 用last节点来记录上一个节点
        // 避免重复值访问
        if (!root) return;
        if (depth == k) {
        ...
        if (root -> left != last) {
            search(root -> left, root, k, depth + 1);
        }
        if (root -> right != last) {
            search(root -> right, root, k, depth + 1);
        }
        // 向上遍历
        if (parents[root -> val] != last) {
            search(parents[root -> val], root, k, depth + 1);
        }
    }
    vector<int> distanceK(TreeNode* root, TreeNode* target, int k) {
        dfs(root);
        search(target, nullptr, k, 0);
        return ans;
    }
};
```
...
### 208. 实现Trie前缀树
>  前缀树 是一种树形数据结构，用于**高效地存储和检索字符串数据集中的键**

- 构建：每个节点都是**存储小写字母**，**构建26叉树即可**
