...
- [综合问题](#综合问题)
  ...
  - [767. 重构字符串](#767-重构字符串)
  - [340. 至多包含K个不同字符的最长子串](#340-至多包含k个不同字符的最长子串)
  - [1044. 最长重复子串*](#1044-最长重复子串)
  - [笔试题： 最长相同子串](#笔试题-最长相同子串)
...
public:
    int lengthOfLongestSubstring(string s) {
        ...
        while (right < n) {
            ...
            while (right < n && !seen.count(s[right])) {
                seen.insert(s[right]);
                right++;
            }
...
```
- 本题问题可以简化为将字符串分为不同的区间， 不同区间内元素不重复。
- 从元素角度出发， 确定每个字母出现的首个位置和最后一个位置
  - 最后就变成了`<first, last>`组合问题， 寻找无交叉的间隔即可
  - 通过每次遍历合并重复/重叠区间即可得到最后寻找的最优区间分割
- 时间复杂度  O(N)
...
public:
    vector<int> partitionLabels(string s) {
        unordered_map<char, vector<int>> firstPos;
...
```
- 进一步简化， 只用记录每个字母最后一次出现的位置即可
- 再做第二次遍历， 来完成区间划分， 本质思路与上面一样， 但省去了很多不必要的操作：
```c++
class Solution {
public:
    vector<int> partitionLabels(string s) {
...
```
- 使用滑动窗口+双指针进行操作：
  - **用hash表记录当前窗口内的字符(记录字符出现的频次)**，当表长度超过k后进行left删除操作
- 时间复杂度 O(N) 空间复杂度 O(N)
- 进阶优化： 使用LRU缓存思想，利用hash表存储字符出现的位置
...
public:
    int lengthOfLongestSubstringKDistinct(string s, int k) {
        ...
        while (right < n) {
            mps[s[right]]++;
            right++;
            // 对于超出的窗口的进行删除 最坏情况下：O(Nk)
            while (mps.size() > k && left < n) {
                mps[s[left]]--;
                if (mps[s[left]] == 0) {
                    mps.erase(s[left]);
                }
...
### 笔试题： 最长相同子串
- 给定字符串， 计算字符串内最长的相同子串， 相同子串之间没有重叠/交叉， 即不考虑子串本身
```
123ABCDEABCF: 3 ABC
```
- 动态规划：
  - 看作两个字符串， 计算最长相同子串（不考虑相同位置、子串本身）
- `dp[i][j]` 
  - `dp[i][j]=dp[i-1][j-1] + 1` 当且仅当两串对应位置相等，且`i!=j`
  - 时间复杂度 O(N^2)
```c++
#include<iostream>
#include<vector>
...
### 187. 重复的DNA序列
> 所有 DNA 都由一系列缩写为 'A'，'C'，'G' 和 'T' 的核苷酸组成;**即字符串只有4种字符**
> 一个函数来找出所有目标子串，**目标子串的长度为 10**，且在 DNA 字符串 s 中出现次数超过一次。

- **固定了子串长度，比[LC1044.最长的重复子串]简单很多**
- `哈希表`直接解题
  - 时间复杂度`O((N-L)L)`, 
  - 线性时间切片 + `N-L`步
- 优化方向：
  - 如何把切片时间进行压缩成常数级：
  - Rabin-Karp字符串编码法
  - 位操作编码法：将字符串每个字符使用位表示，利用位掩码等操作进行快速处理
```c++
class Solution {
public:
    vector<string> findRepeatedDnaSequences(string s) {
        ...
        vector<string> ans(more.begin(), more.end());
        return ans;
    }
};
```
...
public:
    vector<string> findRepeatedDnaSequences(string s) {
        ...
        vector<int> nums;
        for (auto p : s) {
            if (p == 'A') nums.push_back(0);
            if (p == 'C') nums.push_back(1);
            if (p == 'G') nums.push_back(2);
            if (p == 'T') nums.push_back(3);
        }
        int bitmask = 0;
        for (int i = 0; i <= n - 10; i++) {
            if (i == 0) {// init
                ...
                bitmask <<= 2;
                bitmask |= nums[i + 9]; // 添加末尾位置
                bitmask &= ~(3 << (2 * 10)); // 移除前两位
            }
            if (seen.count(bitmask)) {
                more.insert(s.substr(i,10));
            }
            seen.insert(bitmask);
        }
        vector<string> ans(more.begin(), more.end());
