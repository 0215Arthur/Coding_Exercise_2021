...
- [Others](#others)
  ...
  - [5841. 找出到每个位置为止最长的有效障碍赛跑路线](#5841-找出到每个位置为止最长的有效障碍赛跑路线)
...
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        ...
        int n = nums.size();
        target = (sum + target) / 2;
        if (target < 0) return 0; // 新增判断 处理当target为负数的非法情况
...
- 那么状态转移公式为 `dp[k][i] = dp[k - 1][i] + d[k][i - coins[k-1]]`
...
- **贪心+二分搜索**
  - 要**使上升子序列尽可能的长，则我们需要让序列上升得尽可能慢**，因此我们希望每次在上升子序列最后加上的那个数尽可能的小。
    - tails[k]的值表示 **长度为 k + 1 的子序列尾部元素的值**
  - 基于上面的贪心思路，维护一个数组 d[i]，表示长度为 ii 的最长上升子序列的末尾元素的最小值，用len 记录目前最长上升子序列的长度，起始时 lenlen 为 11，d[1] = \textit{nums}[0]d[1]=nums[0]
- 时间复杂度 O(NlogN)
    ...
    int lengthOfLIS(vector<int>& nums) {
        ...
        for (int i = 1; i < nums.size(); i++) {
            if (nums[i] > tail.back()) {
                ...
                // 关键点 即通过二分查找 搜索目标值的下限， 然后进行替换操作； 即找到当前长度下的最小的值， 让序列上升更慢
                while(left <= right) {
...
public:
    int findNumberOfLIS(vector<int>& nums) {
        int n = nums.size(); 
        vector<int> dp(n, 1);// 关键点： 填充为1，以保证整个序列里各个长度都能正常  
        vector<int> cnt(n, 1);
        int ans = 1;
        for (int i = 0; i < n; i ++) {
            for (int j = 0; j < i; j++) {
                // 实际在对比i j dp中的情况 看max在哪个地方取到
                if (nums[j] < nums[i]) {
                    if (dp[i] < dp[j] + 1) {
                        dp[i] = dp[j] + 1;
                        cnt[i] = cnt[j];  // 注意保存结果
...
- 等价于找出给定数组，**每个前缀数组的最长递增子序列的长度**。
- 总体的框架与LC300LIS相同，但是基本的动态规划做法直接超时，要使用基于贪心思想+二分搜索的方法来完成：
  - 使用tail数组记录每个前缀的最大值，即尾部值， 从而构成有序递增数组
  - 当数组值大于/等于tail数组时 直接将数组值添加至tail
    - 那么当前位置上的最长递增子序列为 `tail.size()`
  - 当数组值小于tail数组时， 则需要从tail数组中利用二分搜索法搜索出最后一个小于该值的元素位置
    - 实际计算时采用二分法搜索上阶的方法来实现 upper_bound
  - 并将当前值更新到对应位置上， 那么当前位置上的最长递增子序列即为`idx+1`;
- 时间复杂度 O(N*logN)
- 关键点： **`问题任务转换`**
```c++
class Solution {
public:
    vector<int> longestObstacleCourseAtEachPosition(vector<int>& obstacles) {
        ...
        tail[0] = obstacles[0];
        for (int i = 1; i < n; i++) {
            if (tail.back() <= obstacles[i]) {
                tail.push_back(obstacles[i]);
                ans[i] = tail.size();
            }
            else {
                int idx = upper_bound(tail.begin(), tail.end(), obstacles[i]) - tail.begin();
                ans[i] = idx + 1;
