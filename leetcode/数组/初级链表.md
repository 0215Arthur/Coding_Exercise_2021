...
- [混合操作](#混合操作)
  ...
  - [1171. 从链表中删去总和值为零的连续节点](#1171-从链表中删去总和值为零的连续节点)
...
提示：答案 [1,2,1] 也是正确的。
```
- 本题需要计算连续节点的总和, 肯定能想到使用前缀和来求解
- 考虑到链表的特殊性, 这里需要用前缀和配合哈希表来记录节点对应前缀和
- 为了跳过0节点, 需要通过两次遍历链表来解决:
  - 首先遍历一遍, 计算前缀和
  - 第二次遍历, 进行相同前缀和节点的链接, 两个节点前缀和相同即**说明中间的左开右闭区间总和为0**
- 使用dummy_node来简化代码
- 关键技巧: **`哈希表 + 前缀和`**
