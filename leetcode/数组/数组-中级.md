- [单调栈技巧](#单调栈技巧)
  ...
  - [316. 去除重复字母 [JD.com]](#316-去除重复字母-jdcom)
  - [1081. 不同字符的最小子序列](#1081-不同字符的最小子序列)
  - [321. 拼接最大数 [Hard]](#321-拼接最大数-hard)
  - [456. 132模式](#456-132模式)
...
- 关键点： 在去重的同时， 考虑字典序； 即`babc` 对于b保留第一个还是第二位置， 需要考虑字典序
- 基于单调栈的思路： 
  - 先记录每个字母的出现频次
  - 然后对字符串从头开始遍历， 利用stack栈结构存储保留的字符
    - 当前元素首次出现且当当前元素小于栈顶元素时， 即有更小的字典序时即考虑进行栈顶出栈操作。
    - 为了保证元素均存在，还需要考虑栈顶元素当前的剩余的频次不能为0
- 时间复杂度 O(N)  空间复杂度 O(1)
    ...
    string removeDuplicateLetters(string s) {
        ...
        for (auto & c : s) {
            ...
            if (find(res.begin(), res.end(), c) == res.end()) {
                // 根据单调栈思路进行处理
                // 当当前字符小于栈顶元素 即字典序可能会更小的时候 
                // 且检查出栈后栈顶元素后续是否还会出现 freq>0 则进行出栈操作
...
- 与316题目本质一致， 子序列也要保持字符顺序， 所以跟[LC316]做法一致
- 下面使用单调栈的做法完成
...
public:
    string smallestSubsequence(string s) {
        ...
        for (auto& c : s) {
            if (!visited[c-'a']) {
                while (!res.empty() && res.back() > c && freq[res.back()] > 0) {
                    visited[res.back() - 'a'] = 0; // 注意把出栈元素的标记设为0
                    res.pop_back();
                }
                visited[c-'a'] = 1;
                res.push_back(c);
...
- 题目相对来说非常复杂，包含多个子问题
- 基本的解题过程包括：
  - 对于两个数组选取保序条件下的最大数 **通过单调栈即可完成**
  - 从两个数组中分别选出一定长度的子序列， 即需要遍历不同的长度组合
  - **将子序列进行合并， 合并时需要进行数值比较**， 得到最大组合结果
...
class Solution {
public:
    vector<int> helper(vector<int>& nums, int length) {
        vector<int> res;
    ...
    // 合并需要对每个位置上的元素都进行对比， 若元素相等则需要看后续元素的情况进行对比
    vector<int> merge(vector<int>& nums1, vector<int>& nums2) {
        ...
        while (p1 < nums1.size() || p2 < nums2.size()) {
            if (compare(nums1, nums2, p1, p2) > 0) {
                res[i++] = nums1[p1++];
            }
            else {
                res[i++] = nums2[p2++];
    ...
    // 进行比较  得到两个子序列的大小
    int compare(vector<int> nums1, vector<int> nums2, int p1, int p2) {
        int n1 = nums1.size();
        int n2 = nums2.size();
        while (p1 < n1 && p2 < n2) {
            int d = nums1[p1++] - nums2[p2++];
            if (d != 0) return d; // < 0  > 0
        }
        return (n1 - p1) - (n2 - p2); // 对比剩余长度的情况
    }
    ...
    vector<int> maxNumber(vector<int>& nums1, vector<int>& nums2, int k) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            vector<int> x = helper(nums1, i);
            vector<int> y = helper(nums2, k - i);
            vector<int> tmp = merge(x, y);
            if (compare(tmp, res, 0, 0) > 0) {
                res.swap(tmp);
            }
        }
        return res;
...
- 解题思路： 从每个位置开始搜索， 适用哈希表进行存储当前搜索下的结果：
  - dw: 存储给定的单词的哈希 注意： 由于存在重复单词， 要用unordered_map来存储， 保存对应出现的频次
  - win: 记录当前搜索窗口内的哈希信息；
    - 当win与dw完全匹配时，则记录下当前窗口的起始位置；
    - 然后从一个位置继续遍历搜索
- 这种基础做法： 时间复杂度在 O(N^2) M : 单词数量 * L 单词长度

```c++
class Solution {
public:
    vector<int> findSubstring(string s, vector<string>& words) {
        unordered_map<string, int> dw;
        for (auto & p : words) {
            dw[p]++;
        }
        int _len = words[0].size();
        int i = 0;
        int n = s.size();
        int total = _len * words.size();
        vector<int> ans;
        // 单词存在重复
        while (i <= n - total) {
            //cout << "i:" << i << endl;
            string tmp = s.substr(i, _len);
            int j = 1;
            int index = i;
            int valid = 0;
            unordered_map<string, int> win;
            while (dw.find(tmp) != dw.end()) {
                win[tmp]++;
...
```
- 本题关键还是在于滑动窗口
  - 上面的方法本质还是暴力算法， 没有利用滑动窗口的特性
- 重新设计滑动窗口算法：
  - left / right； 记录匹配成功的次数， 当某个单词的匹配次数大于目标频次时，进行窗口左侧移动；
    - 每次移动按照单词长度进行移动，从而显著提升效率
    - 当匹配次数等于单词数量时， 即完成了滑窗匹配，进行left保存
  - 由于单词的长度是相等的， 那么起始窗口的位置应该在[0,word_len] 范围之内
- 经过以上优化： 时间复杂度为： O(N * L) L为单词的长度
...
public:
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        for (int i = 0; i < _len; i++) {
            ...
            unordered_map<string, int> win;
            while (right + _len <= n) {
                string tmp = s.substr(right, _len);
                right += _len;
                if (dw.find(tmp) != dw.end()) {
                    win[tmp]++;
                    cnt++;
                    // 调整窗口内的计数；
                    // 当出现次数大于对应dw中的频次时即进行左侧删除操作
                    while (win[tmp] > dw[tmp]) {
                        string ts = s.substr(left, _len);
                        cnt--;
                        win[ts]--;
                        left += _len;
                    }
                ...
                else { // 当出现不属于words中的词时候，进行状态清空操作
                    left = right;
                    cnt = 0;
                    win.clear();
                }
