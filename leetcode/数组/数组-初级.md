- [遍历问题](#遍历问题)
  ...
  - [1736.  替换隐藏数字得到的最晚时间](#1736--替换隐藏数字得到的最晚时间)
- [旋转问题](#旋转问题)
    - [48.旋转图像](#48旋转图像)
  - [189. 旋转数组](#189-旋转数组)
- [位运算技巧](#位运算技巧)
...
### 1736.  替换隐藏数字得到的最晚时间
> 字符串 time ，格式为 hh:mm（小时：分钟），其中某几位数字被隐藏（用 ? 表示）。
有效的时间为 00:00 到 23:59 之间的所有时间，包括 00:00 和 23:59 。
替换 time 中隐藏的数字，返回你可以得到的最晚有效时间。

- 基于模拟完成即可
- 进行诸位判断：
  - 在hour位上的需要满足`0~23`的约束，因此需要进行情况判断，分别设置最优情况
  - 在minute位上的两个数字直接设为最大值即可 `59`
...
```
- 基础做法: 复制原数据进行排序处理, 对比排序后的数组原数组的差异位置, 两个差异位置即为无序段
...
```
- 进阶: 如何设计时间复杂度为O(N)的算法
    - 尝试分别寻找目标区间的左右边界, 通过遍历从左向右搜索右边界, 即找到值小于左侧最大值的位置
    - 从右向左搜索左边界, 即找到值大于右侧最小值的位置
    - 即无序子数组中最小元素的正确位置可以决定左边界，最大元素的正确位置可以决定右边界
  - 注意临界条件: 对于有序数组应该返回0, 通过设置`left/right = -1`初始值来判断.
- 时间复杂度 O(N)
